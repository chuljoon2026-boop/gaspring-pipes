...
export default function ARView({ onClose }: { onClose: () => void }) {
  ...
  return <div ref={overlay} className={'floor-ar underground-ar ' + (active ? 'is-active ' : '') + (capability === 'camera' ? 'camera-mode' : '')} data-ar-phase={phase} data-ar-mode="underground" data-ar-backend={capability}>
    <div className="floor-ar-stage" aria-label="지면 아래 배관 미리보기">
      <Suspense fallback={<div className="floor-ar-loading">배관을 준비하고 있습니다</div>}>
        {capability === 'camera' ? <CameraARScene ref={cameraScene} settings={settings} onPhase={onPhase} onReady={onReady} onError={setError} />
          : capability !== 'checking' && <FloorARScene ref={scene} settings={settings} onPhase={onPhase} onReady={onReady} />}
      </Suspense>
    </div>
