...
</style></head><body><div class="toolbar"><button onclick="window.print()">QR 안내판 인쇄 / PDF 저장</button></div><main class="marker"><div class="tag">YS-001 · 여수산단</div><h1>배관 조회 · 현장 신고</h1><p class="sub">카메라로 QR을 스캔하세요.<br>도로 하부 배관 · 단면 · 교차부 확인</p><div class="qr">${svg}</div><div class="location"><strong>현장 YS-001</strong></div><a class="url" href="${safeUrl}">${safeUrl}</a>${warning ? `<p class="warning">${escapeHtml(warning)}</p>` : ''}</main></body></html>`
