...
QR 진입 → 공사 안내(공개) / 현장 업무(접수번호 확인) → 지하 투시 AR / 3D 배관 보기.
첫 화면은 한빛산단 앞 도로라는 예시 현장입니다. 도로/보도/건물로 구성한 지형에서 지상/지하를 전환합니다. 실제 공사 사이트 연결 및 기관 신고 전송은 없습니다. 공사 안내/위험요소 신고는 누구나 이용하고, 배관 3D/AR은 접수번호 확인 후 이용합니다. 시험 정보는 `DEMO-2026-001` / `1234`이며 자동입력 후 현장 확인 버튼으로 진입합니다. 이 확인 화면은 클라이언트 데모이며 서버 인증이 아닙니다.
...
공식 웹 로그인은 아이디/비밀번호입니다. 접수번호별 비밀번호를 발급하는 제도는 확인되지 않았습니다. 이 데모의 **작업 현장 확인**은 공사 신고 접수번호 `DEMO-2026-001` / 현장 비밀번호 `1234`로 기능을 여는 제안 화면입니다. 실제 번호 형식이나 EOCS 인증을 재현한 것이 아닙니다. 시험용 정보 자동입력 후 현장 확인을 누르면 **지하 투시 AR / 3D 배관 보기**를 선택합니다. 직접 주소도 같은 확인을 거칩니다.

공개 메뉴는 **공사 안내**, 시민 버튼은 **위험요소 신고**, 작업 메뉴는 **현장 업무**입니다. 안전신문고의 안전신고 용어를 참고했습니다: https://www.mois.go.kr/frt/sub/a06/b10/safetyReport/screen.do
...
현장 업무 탭에서 접수번호 `DEMO-2026-001`, 현장 비밀번호 `1234`를 사용합니다. **시험용 정보 자동입력** 후 **현장 확인**을 누릅니다. 배관/AR 직접 주소도 현장 확인을 거치며, 로그아웃하면 다시 확인해야 합니다.
