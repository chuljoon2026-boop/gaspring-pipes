...
카메라 시작 한 번으로 자동 실행합니다. WebXR CPU 깊이 정보가 제공되면 현재 화면의 실제 표면 높이를 지면 기준과 비교하여 배관을 지면에만 표시합니다. 깊이 정보가 없으면 자동으로 일반 카메라 영상 분할로 전환합니다. 직접 영역을 지정하는 단계는 없습니다.

일반 카메라는 SegFormer B0 / ADE20K 양자화 모델을 Web Worker / ONNX Runtime WASM으로 실행합니다. 바닥 / 도로 / 보도 / 흙 / 잔디 / 모래 / 러그만 허용하고, 사물 경계와 불확실한 영역은 제외합니다. 카메라 crop과 인식 영상의 좌표를 일치시키고, 방향 변화 / 오래된 결과 / 화면 변화에 따른 잔상을 제거합니다. 영상은 서버로 전송하지 않습니다. 첫 실행 시 약 4.4 MB 모델과 약 11.2 MB WASM을 받으며 이후 캐시합니다. 초기 인식과 갱신 속도는 기기 성능에 따라 다릅니다.

카메라 화면에는 관로명 / 식별번호 / 중심 심도 / 관경 / 재질을 표시합니다. 일반 카메라의 지면 높이는 1.4 m 촬영 높이 가정이며 이동 거리나 실제 매설물을 측정하지 않습니다. 영상 분할에는 오인식 가능성이 있고, 실제 기기에서의 인식 정확도는 별도 확인이 필요합니다. 검증은 모의 XR 깊이 / 실제 사진을 입력한 브라우저 카메라 스트림 / 세션 종료 및 자동 전환으로 수행합니다.

모델 출처와 고정 revision은 `public/models/segformer/SOURCE.txt`에 기록했습니다. SegFormer는 연구 / 평가용 라이선스이며 이번 공모전 시연에 사용합니다. 배포물에 원문 `LICENSE.txt`를 포함합니다.


## AR 경계 / 심도 표현 보완

이진 마스크의 확대 및 픽셀 삭제를 제거하고 지면 신뢰도 / 양선형 보간 / 좁은 경계 페더링으로 투명도를 연속 처리합니다. 실제 깊이 샘플은 이전과 같은 96×128 간격으로 읽으며 출력 마스크만 192×256으로 보간합니다. 그림자처럼 보이는 강한 자체 발광을 줄이고 원통 명암을 강화했습니다.

심도 기준선은 지면 0 m / 0.5 m 눈금 / 배관 중심 심도를 보여주는 도식입니다. 도식은 배관 표면과 구분하여 사물 위에서도 읽을 수 있으며 실제 측량 결과를 주장하지 않습니다. 긴 직선 관로에서는 화면에 보이는 구간으로 기준선을 옮기되 배관의 위치는 변경하지 않습니다. 실제 인식 경계의 정확도가 모델 출력 해상도보다 높아진다는 의미는 아닙니다. 실기기 / 역광 / 움직이는 사물은 추가 확인이 필요합니다.

선택 관로의 가려진 구간에는 낮은 불투명도의 점선 중심선만 표시합니다. 이는 가상 GIS 연결 관계를 설명하는 기호이며 사물 위에 배관 본체를 얹어 그리지 않습니다. 해당 시설 레이어를 끄면 심도 기준선과 점선도 함께 숨깁니다.
